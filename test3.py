...
def _write_helper(field_permissions, entity, field_key, field, field_value):
  
  is_writable = (field_key in field_permissions) and (field_permissions[field_key]['writable'])
  
  if _is_structured_field(field) and not is_writable:
  
     util.logger('is structured, repeated and not writable - recursion %s.%s' % (entity.__class__.__name__, field_key))
 
     new_entity = getattr(entity, field_key)
 
     for new_field_key, new_field in field.get_model_fields().items():
         if field._repeated:
           for i,new_entity_item in enumerate(new_entity):
              _write_helper(field_permissions[field_key], new_entity_item, new_field_key, new_field, getattr(field_value[i], new_field_key))
         else:
           _write_helper(field_permissions[field_key], new_entity, new_field_key, new_field, getattr(field_value, new_field_key))
  else:
    
    if is_writable:
       util.logger('writable_helper setting %s.%s = %s' % (entity.__class__.__name__, field_key, field_value))
       try:
...
def write(entity, values):
    ...
    for value_key, value in values.items():
        if value_key in entity_fields: # check if the value is in entities field list
          ...
          _write_helper(entity._field_permissions, entity, value_key, value_field, value)
