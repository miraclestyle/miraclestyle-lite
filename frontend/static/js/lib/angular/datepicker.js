angular.module('app.ui.datepicker', ['app.ui.position'])
...
.controller('DatepickerController', ['$scope', '$attrs', '$parse', '$interpolate', '$log', 'dateFilter', 'datepickerConfig', function($scope, $attrs, $parse, $interpolate, $log, dateFilter, datepickerConfig) {
  ...
  $scope.select = function( date, noclose) {
 
    if ( this.datepickerMode === self.minMode ) {
      var dt = ngModelCtrl.$modelValue ? new Date(ngModelCtrl.$modelValue) : new Date();
...
.directive('daypicker', ['dateFilter', function (dateFilter) {
  return {
    restrict: 'EA',
    replace: true,
    templateUrl: ui_template('datepicker/day.html'),
    require: '^datepicker',
...
.directive('monthpicker', ['dateFilter', function (dateFilter) {
  return {
    restrict: 'EA',
    replace: true,
    templateUrl: ui_template('datepicker/month.html'),
    require: '^datepicker',
    link: function(scope, element, attrs, ctrl) {
      ctrl.mode = {
        step: { years: 1 },
...
.directive('yearpicker', ['dateFilter', function (dateFilter) {
  return {
    restrict: 'EA',
    replace: true,
    templateUrl: ui_template('datepicker/year.html'),
    require: '^datepicker',
    link: function(scope, element, attrs, ctrl) {
      ctrl.mode = {
...
.directive( 'datepicker', function () {
  return {
    restrict: 'EA',
    replace: true,
    templateUrl: ui_template('datepicker/datepicker.html'),
    scope: {
      datepickerMode: '=?',
      dateDisabled: '&'
    },
...
function ($compile, $parse, $document, $position, dateFilter, datepickerPopupConfig) {
  return {
    ...
    link: function(scope, element, attrs, ngModel) {
      ...
      // Inner change
      scope.dateSelection = function(dt, noclose) {
        ...
        if ( closeOnDateSelection && !noclose) {
          scope.isOpen = false;
        }
      };
	  ...
	  scope.setTimes = function (date)
	  {
	  	 return this.times.str = dateFilter(date, 'HH:mm:ss');
	  };
	  
	  scope.validateTime = function ()
	  {
	  	  var ts = this.times.str.split(':');
	  	  
	  	  if (ts.length != 3) return false;
	  	  var fail = false;
	  	  angular.forEach(ts, function (value) {
	  	  	 if (value.length != 2)
	  	  	 {
	  	  	 	fail = true;
	  	  	 }
	  ...
	  scope.parseTimes = function(dt)
	  { 
	  	 
	  	 try
	      {
	        var t = this.validateTime();
	        
	        if (!t) return false;
	        
	      	dt.setHours(t[0], t[1], t[2]);
      ...
      scope.changeTime = function ()
	  {
	  	  ...
	  	  var newd = new Date(scope.date);
	  	  this.parseTimes(newd);
	      scope.dateSelection(newd, 1);
	  };
...
.directive('datepickerPopupWrap', function() {
  return {
    ...
    replace: true,
    transclude: true,
    templateUrl: ui_template('datepicker/popup.html'),
