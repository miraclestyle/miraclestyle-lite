...
(function( $ ) {
    ...
    function JSONEditor(target, json, onchange, onpropertyclick, propertyElement, valueElement) {
        ...
        if (!$(opt.target).data('jsoneditor'))
        {
             $(opt.target).data('jsoneditor', true);
             $(opt.target).bind('blur focus', '.property, .value', function() {
    ...
    function construct(opt, json, root, path) {
        ...
        if (isObject(json) || isArray(json)) {
            addListAppender(root, function () {
                $(opt.target).trigger('jsoneditor.beforeAdd', [opt, json, root, path]);
                addNewValue(json);
                $(opt.target).trigger('jsoneditor.afterAdd', [opt, json, root, path]);
                construct(opt, json, root, path);
                opt.onchange(parse(stringify(opt.original)));
                $(opt.target).trigger('jsoneditor.finalAdd', [opt, json, root, path]);
