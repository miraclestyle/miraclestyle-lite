...
(function() {
    ...
    MdAdialogDirective.$inject = ["$$rAF", "$mdTheming"];

    /**
     * @ngdoc service
     * @name $mdAdialog
     * @module material.components.adialog
     *
     * @description
     * `$mdAdialog` opens a dialog over the app and provides a simple promise API.
     *
     * ### Restrictions
     *
     * - The dialog is always given an isolate scope.
     * - The dialog's template must have an outer `<md-adialog>` element.
     ...
     *
     *   function EmployeeEditor($scope, $mdAdialog) {
     *     var alert;
     *
     ...
     *     // Normally this is not needed, just use '$mdAdialog.hide()' to close
     *     // the most recent dialog popup.
     *
     *     function closeAlert() {
     *       $mdAdialog.hide( alert, "finished" );
     *       alert = undefined;
     *     }
     *
     ...
     *
     *   function GreetingController($scope, $mdAdialog, employee) {
    ...
    /**
     * @ngdoc method
     * @name $mdAdialog#alert
     *
     * @description
     * Builds a preconfigured dialog with the specified message.
     *
     * @returns {obj} an `$mdAdialogPreset` with the chainable configuration methods:
     *
     * - $mdAdialogPreset#title(string) - sets title to string
     * - $mdAdialogPreset#content(string) - sets content / message to string
     * - $mdAdialogPreset#ok(string) - sets okay button text to string
     *
     */

    /**
     * @ngdoc method
     * @name $mdAdialog#confirm
     *
     * @description
     ...
     * - $mdAdialogPreset#content(string) - sets content / message to string
     * - $mdAdialogPreset#ok(string) - sets okay button text to string
     * - $mdAdialogPreset#cancel(string) - sets cancel button text to string
     *
     */
     ...
     *
     * @param {object} optionsOrPreset Either provide an `$mdAdialogPreset` returned from `alert()`,
     ...
     *     finished.
     *
     * @returns {promise} A promise that can be resolved with `$mdAdialog.hide()` or
     ...
     *
     * @description
     * Hide an existing dialog and resolve the promise returned from `$mdAdialog.show()`.
     *
    ...
    /**
     * @ngdoc method
     * @name $mdAdialog#cancel
     *
     * @description
     * Hide an existing dialog and reject the promise returned from `$mdAdialog.show()`.
     *
    ...
    function MdAdialogProvider($$interimElementProvider) {
        ...
        return $$interimElementProvider('$mdAdialog')
            .setDefaults({
        ...
        function advancedDialogOptions($mdAdialog) {
            return {
                template: [
                    '<md-adialog aria-label="{{ dialog.ariaLabel }}">',
                    '<md-content>',
                    '<h2>{{ dialog.title }}</h2>',
                    '<p>{{ dialog.content }}</p>',
                    '</md-content>',
                ...
                controller: function mdAdialogCtrl() {
                    this.hide = function() {
                        $mdAdialog.hide(true);
                    };
                    this.abort = function() {
        ...
        function dialogDefaultOptions($timeout, $rootElement, $compile, $animate, $mdAria, $document,
            $mdUtil, $mdConstant, $mdTheming, $$rAF, $q, $mdAdialog) {
            return {
                hasBackdrop: true,
                isolateScope: true,
                onShow: onShow,
                ...
                transformTemplate: function(template) {
                    return '<div class="md-adialog-container">' + template + '</div>';
                }
            };
            ...
            function dialogPopIn(container, options) {
                ...
                if (!clickElement && options.inDirection) {
                    dialogEl.addClass('visible')
                        .css($mdConstant.CSS.TRANSFORM, 'translate3d(' + (options.inDirection === 'right' ? '-' : '') + '100%, 0px, 0px)');
                    $$rAF(function() {
                        dialogEl.addClass('transition-in-' + options.inDirection)
            ...
            function dialogPopOut(container, options) {
                ...
                var dialogEl = container.find(discoverDirective(options)),
                    ...
                    type,
                    clickElement = options.popInTarget && options.popInTarget.length && options.popInTarget;
                if (!clickElement && options.outDirection) {
                    dialogEl.addClass('transition-out-' + options.inDirection).removeClass('transition-in-' + options.inDirection)
                        .css($mdConstant.CSS.TRANSFORM, 'translate3d(' + (options.inDirection === 'right' ? '' : '-') + '100%, 0px, 0px)');
                    return dialogTransitionEnd(dialogEl);
                } else {
    ...
    MdAdialogProvider.$inject = ["$$interimElementProvider"];
...
(function() {
    ...
    function MdAsimpleDialogProvider($$interimElementProvider) {
        ...
        dialogDefaultOptions.$inject = ["$timeout", "$rootElement", "$compile", "$animate", "$mdAria", "$document", "$mdUtil", "$mdConstant", "$mdTheming", "$$rAF", "$q", "$mdAsimpleDialog", "mdEscFactory"];
        ...
        /* @ngInject */
        function dialogDefaultOptions($timeout, $rootElement, $compile, $animate, $mdAria, $document,
            $mdUtil, $mdConstant, $mdTheming, $$rAF, $q, $mdAsimpleDialog, mdEscFactory) {
            return {
                hasBackdrop: true,
                isolateScope: true,
                onShow: onShow,
