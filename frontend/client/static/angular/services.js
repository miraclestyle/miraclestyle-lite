...
function($http, DSCacheFactory, $cacheFactory, GLOBAL_CONFIG, Helpers) {
  ...
  return {
    ...
    cached : function(key, action, model, data, success, error, config) {
      ...
      var loading_key = key + '_loading', callbacks_key = loading_key + '_success_callbacks', callbacks = in_memory.get(callbacks_key), loading = in_memory.get(loading_key), cached = cache.get(key);
