angular.module('app').run(function ($window, modelsEditor, modelsMeta, $q,
  modelsConfig) {
      ...
      fields.addresses.ui = {
        specifics: {
          ...
          beforeSave: function ($scope, info) {
            ...
            if (updated_address.region && (!updated_address._region ||
                (updated_address.region !== updated_address._region
                  .key))) {
              var promise = models['13'].get(updated_address.region);

              promise.then(function (response) {
                if (response.data.entities.length) {
                  updated_address._region = response.data.entities[
                    0];
                }
            ...
            if (updated_address.country && (!updated_address._country ||
                (updated_address.country !== updated_address._country
                  .key))) {
              var promise = models['12'].actions.search(
                undefined, {
                  cache: true
                });
              promise.then(function (response) {
            ...
            angular.forEach($scope.parentArgs, function (address) {
              if (updated_address.default_billing ||
                updated_address.default_shipping) {
