...
class Catalog(orm.BaseExpando):
  ...
  publish_date = orm.SuperDateTimeProperty('4', required=True, searchable=True)  # @todo This field is currently not required however, it could be beneficial for search, sorting, ranking, etc.! It can be renamed to published. In the future it could be used for scheduling publishing (with name publish_date)!
