...
class Template(ndb.BasePoly):
  ...
  def initiate(cls, context):
    entity_key = ndb.Key(urlsafe=context.input.get('entity_key'))
    entity = entity_key.get()
    action_key = ndb.Key(urlsafe=context.input.get('action_key'))
    user_key_str = context.input.get('user_key')
    user = None
    if user_key_str:
      user_key = ndb.Key(urlsafe=context.input.get('user_key'))
      user = user_key.get()
    ...
    if templates_count:
      templates_per_task = int(math.ceil(templates_count / settings.TEMPLATES_PER_TASK))
      for i in range(0, templates_per_task + 1):
        cursored_templates = outlet_templates[settings.TEMPLATES_PER_TASK * i:settings.TEMPLATES_PER_TASK * (i + 1)]
        for template in cursored_templates:
